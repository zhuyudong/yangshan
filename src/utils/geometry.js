...
export function createFace(width, height, depth, color) {
  let zeroX = width / 2
  let zeroY = height / 2
  let zeroZ = depth || 0.5
  let cornerPositions = [
...
export function createWing(topWidth, bottomWidth, height, depth) {
...
export function createLongCube(width, height, depth, repeatCount) {
  let zeroX = width / 2
  let zeroY = height / 2
  let zeroZ = depth / 2
...
export function createSphere(radius, divideByYAxis, divideByCircle) {
...
export function getElementsCountPerVertex(attribute) {
  let result = 3
  switch (attribute) {
    case 'colors':
      result = 4
...
export function getArrayTypeByAttribName(attribute) {
...
export function transformIndicesToUnIndices(vertex) {
